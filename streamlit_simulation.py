...
class BicycleSimulation:
    def __init__(self):
        ...
        self.balance = 50000  # Startguthaben: 50.000€
    ...
    def produce_bicycles(self, production_plan):
        ...
        for bike_type, quantity in production_plan.items():
            ...
            if skilled_hours_used + skilled_hours_needed > skilled_capacity:
                ...
                st.warning(f"Nicht genügend Facharbeiterkapazität für {quantity} {bike_type}. Maximal möglich: {max_possible}")
            ...
            if unskilled_hours_used + unskilled_hours_needed > unskilled_capacity:
                ...
                st.warning(f"Nicht genügend Hilfsarbeiterkapazität für {quantity} {bike_type}. Maximal möglich: {max_possible}")
                quantity = max_possible
            ...
            for component_type, component_name in recipe.items():
                ...
                total_available = self.inventory_germany.get(component_name, 0) + self.inventory_france.get(component_name, 0)
                
                if total_available < quantity:
                    can_produce = min(can_produce, total_available)
                    st.warning(f"Nicht genügend {component_name} für {quantity} {bike_type}. Vorhanden: {total_available}")
    ...
    def distribute_to_markets(self, distribution_plan):
        ...
        for market, bikes in distribution_plan.items():
            ...
            for bike_type, quantity in bikes.items():
                ...
                if total_available < quantity:
                    st.warning(f"Nicht genügend {bike_type} auf Lager. Vorhanden: {total_available}, Benötigt: {quantity}")
                ...
                if market == 'muenster':
                    ...
                    shipping_cost += from_germany * 150  # 150€ pro Fahrrad von DE nach Münster
                    ...
                    if from_germany < quantity:
                        ...
                        shipping_cost += from_france * 500  # 500€ pro Fahrrad von FR nach Münster
                    ...
                    self.inventory_france[bike_type] -= from_france
                    shipping_cost += from_france * 150  # 150€ pro Fahrrad von FR nach Toulouse
                    ...
                    if from_france < quantity:
                        ...
                        self.inventory_germany[bike_type] -= from_germany
                        shipping_cost += from_germany * 500  # 500€ pro Fahrrad von DE nach Toulouse
                
                # Aktualisiere die Fahrräder auf dem Markt
                shipped_quantity = from_germany + from_france
                self.markets[market]['bicycles'][bike_type] = self.markets[market]['bicycles'].get(bike_type, 0) + shipped_quantity
    ...
    def calculate_storage_usage(self):
        ...
        return {
            'germany': {
                ...
                'percentage': (germany_usage / self.storage_space['germany']) * 100 if self.storage_space['germany'] > 0 else 0
            },
            'france': {
                'used': france_usage,
                'total': self.storage_space['france'],
                'percentage': (france_usage / self.storage_space['france']) * 100 if self.storage_space['france'] > 0 else 0
            }
        }
    
    def simulate_sales(self):
        """
        Simuliert Verkäufe am Ende eines Quartals (alle 3 Monate)
        """
        if self.current_month % 3 != 0:
            return {'status': 'No sales this month'}
    ...
    def generate_monthly_report(self):
        ...
        inventory_summary = {
            'materials': {
                'germany': {k: v for k, v in self.inventory_germany.items() if k not in ['damenrad', 'e_bike', 'e_mountainbike', 'herrenrad', 'mountainbike', 'rennrad']},
                'france': {k: v for k, v in self.inventory_france.items() if k not in ['damenrad', 'e_bike', 'e_mountainbike', 'herrenrad', 'mountainbike', 'rennrad']}
            },
            'bicycles': {
                'germany': {k: v for k, v in self.inventory_germany.items() if k in ['damenrad', 'e_bike', 'e_mountainbike', 'herrenrad', 'mountainbike', 'rennrad']},
                'france': {k: v for k, v in self.inventory_france.items() if k in ['damenrad', 'e_bike', 'e_mountainbike', 'herrenrad', 'mountainbike', 'rennrad']}
            },
...
if st.session_state.show_report:
    ...
    if sim.monthly_reports:
        ...
        with col2:
            ...
            st.metric("Personal", f"{report['staff']['total']} ({report['staff']['skilled']} Fach, {report['staff']['unskilled']} Hilfs)")
        ...
        with col1:
            ...
            st.write(f"Deutschland: {report['storage']['germany']['used']:.2f} von {report['storage']['germany']['total']} m ({report['storage']['germany']['percentage']:.1f}%)")
        
        with col2:
            st.progress(report['storage']['france']['percentage'] / 100)
            st.write(f"Frankreich: {report['storage']['france']['used']:.2f} von {report['storage']['france']['total']} m ({report['storage']['france']['percentage']:.1f}%)")
    ...
    if st.session_state.current_tab == "Übersicht":
        ...
        with col1:
            ...
            for bike_type in bike_types:
                st.write(f"{bike_type.replace('_', ' ').title()}: {sim.markets['muenster']['bicycles'].get(bike_type, 0)}")
        
        with col2:
            st.write("Toulouse")
            for bike_type in bike_types:
                st.write(f"{bike_type.replace('_', ' ').title()}: {sim.markets['toulouse']['bicycles'].get(bike_type, 0)}")
        ...
        with col1:
            ...
            st.write(f"Genutzt: {storage_usage['germany']['used']:.2f} von {storage_usage['germany']['total']} m ({storage_usage['germany']['percentage']:.1f}%)")
        ...
        with col2:
            ...
            st.write(f"Genutzt: {storage_usage['france']['used']:.2f} von {storage_usage['france']['total']} m ({storage_usage['france']['percentage']:.1f}%)")
            ...
            if sim.inventory_germany.get(item, 0) > 0 or sim.inventory_france.get(item, 0) > 0:
                ...
                with col4:
                    ...
                    if transfer_direction != "Keine":
                        ...
                        max_transfer = sim.inventory_germany.get(item, 0) if from_warehouse == "germany" else sim.inventory_france.get(item, 0)
        ...
        if transfer_initiated:
            if st.button("Transfer durchführen"):
                ...
                if result['fee'] > 0:
                    st.success(f"Transfer erfolgreich durchgeführt! Verwaltungsgebühr: {format_currency(result['fee'])}")
                    st.session_state.monthly_action_taken = True
                else:
        ...
        with col1:
            ...
            fire_skilled = st.number_input("Entlassen", min_value=0, max_value=sim.skilled_workers, value=0, step=1, key="fire_skilled")
        
        with col2:
            st.write("**Hilfsarbeiter**")
            hire_unskilled = st.number_input("Einstellen", min_value=0, value=0, step=1, key="hire_unskilled")
            fire_unskilled = st.number_input("Entlassen", min_value=0, max_value=sim.unskilled_workers, value=0, step=1, key="fire_unskilled")
        ...
        new_salary_costs = (new_skilled_total * sim.worker_salaries['skilled']) + (new_unskilled_total * sim.worker_salaries['unskilled'])
        current_salary_costs = (sim.skilled_workers * sim.worker_salaries['skilled']) + (sim.unskilled_workers * sim.worker_salaries['unskilled'])
                ...
                with col1:
                    ...
                    for component_type, component_name in recipe.items():
                        ...
                        if component_name is None:
                            ...
                            st.write(f"- {component_type.title()}: {component_name.split('_')[1].title()} ({total_available} verfügbar)")
        ...
        - **Lagermiete**: 5.000 € für Deutschland, 2.500 € für Frankreich (pro Quartal)
        ...
        - **Transport zu Märkten**:
          - Deutschland → Münster: 150 € pro Fahrrad
          - Deutschland → Toulouse: 500 € pro Fahrrad
          - Frankreich → Toulouse: 150 € pro Fahrrad
          - Frankreich → Münster: 500 € pro Fahrrad
